...
class g_announecement{


  Future<List> getData_Announecement(filter) async {
    int errorCode = 0;
    await AppHelper().getConnect().then((value){if(value == 'ConnInterupted'){
      errorCode = 2; return false;}});
    http.Response response = await http.get(  Uri.parse(applink+"mobile/api_mobile.php?act=getData_Announecement&"
        "filter="+filter)).
